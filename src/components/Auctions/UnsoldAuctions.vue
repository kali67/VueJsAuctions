  ...
  export default {
    ...
    methods: {
      getCreatedAuctions: function () {
        this.$http.get('http://localhost:4941/api/v1/auctions?seller=' + window.sessionStorage.getItem("userId")
          + '&status=expired&startIndex=' + this.startIndex + '&count='+this.size)
          .then((response) => {
      ...
      isLastPage: function() {
        ...
        this.$http.get('http://localhost:4941/api/v1/auctions?seller=' + window.sessionStorage.getItem("userId")
          +'&status=expired&startIndex=' + index + '&count='+this.size)
          .then((response) => {
